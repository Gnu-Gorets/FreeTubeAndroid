...
assert.match(bridge, /pendingExternalLaunches\[relayToken\] = PendingExternalLaunch\(title\)/)
...
assert.match(bridge, /if \(isManifest\) "application\/dash\+xml" else "video\/mp4"/)
assert.match(bridge, /removeSuffix\("\.mpd"\)\?\.removeSuffix\("\.mp4"\)/)
assert.match(bridge, /launchExternalPlayer\(relayUrl, pending\.title/)
...
assert.match(video, /const useAdaptiveStreams = !selectedFormat && Boolean\(selectedAdaptiveVideo && selectedAdaptiveAudio\)/)
