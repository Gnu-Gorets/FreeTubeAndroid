...
test('native adaptive download exposes processing mux lifecycle', () => {
  ...
  assert.ok(downloadServiceSource.includes('validateMuxInput(videoFile, "video")'))
  assert.ok(downloadServiceSource.includes('verifyMuxOutput(outputFile)'))
  assert.ok(downloadServiceSource.includes('delete(item.optString("targetUri"))'))
  assert.ok(downloadServiceSource.includes('takeIf { it > 0 }?.let { 0.0 }'))
  assert.ok(downloadServiceSource.includes('ffmpegSessions.keys.forEach(::cancelFfmpeg)'))
  assert.ok(downloadServiceSource.includes('FFmpegKit.executeAsync'))
